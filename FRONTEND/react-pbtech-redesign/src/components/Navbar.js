...
function Navbar({ cartCount }) {
  ...
  const [
    ...
    setDepartmentsSearchDropdownVisible,
  ] = useState(false);
  const [
    departmentsSearchSelectedDepartments,
  ...
  const departmentsSearchHandleCheckboxChange = (event) => {
    ...
    if (event.target.checked) {
      setDepartmentsSearchSelectedDepartments((prevDepartments) => [
        ...prevDepartments,
        department,
      ]);
    } else {
  ...
  const [wasDropdownVisible, setWasDropdownVisible] = useState(false);

  useEffect(() => {
    if (departmentsSearchDropdownVisible) {
      setSearchBarInputValue(departmentsSearchSelectedDepartments.join(", "));
      setWasDropdownVisible(true);
    } else if (!toggleSearchBarVisible) {
      setWasDropdownVisible(false);
    }
  }, [
    departmentsSearchSelectedDepartments,
    departmentsSearchDropdownVisible,
    ...
    <div className="navbar">
      <div className="navbar-top">
        ...
        <div className="navbar-top-left-item">
          <button className="navbar-top-item">
            <FontAwesomeIcon
              icon={faMobileAlt}
      ...
      <div className="navbar-middle">
        ...
        <div className="search">
          <div className="search-options">
            <button
              className="search-by-department"
              onClick={departmentsSearchToggleDropdown}
            >
              Search by Department
              <img src={departmentPlus} alt="Department Plus" />
            </button>
            <span className="selected-departments">
              {" "}
              • {departmentsSearchSelectedDepartments.length} Departments
              Selected
            </span>
            {departmentsSearchDropdownVisible && (
                  ...
                  <div key={index} className="department-row">
                    <div className="department-header">
                      ...
                      <input
                        ...
                        onChange={departmentsSearchHandleCheckboxChange}
                      />
                    </div>
                    {department.items.map((item, itemIndex) => (
                      <div key={itemIndex} className="department-item">
                        {item}
                      </div>
                    ))}
                  </div>
              ...
              <div className="search-input-container">
                ...
                >
                  {searchBarOptions.map((option, index) => (
                    <div
                      key={index}
                      onClick={() => searchBarHandleOptionClick(option)}
                    >
                      {option}
                    </div>
                  ))}
                ...
                <Button
                  className={`btn btn-blue${
                    searchBarInputValue ? "" : ":invalid"
                  } btn-small`}
                >
                  <span className="button-text">Search Store</span>
                </Button>
              </div>
            </div>
          </div>
        ...
        <div className="client-buttons">
          <Button className="btn btn-blue btn-small-navbar btn-no-hover">
            <FontAwesomeIcon icon={faUser} style={{ color: "#ffffff" }} />{" "}
            <span className="button-text-navbar"> Log In</span>
          </Button>
      ...
      <div className="navbar-middle-hiden">
        <div className="toggle-search-container">
          <div className="toggle-search-bar-container">
            <div>
              <div
                className={`toggle-search-bar ${
                  toggleSearchBarVisible ? "open" : ""
                }`}
              >
                <input
                  ref={searchBarRef}
            ...
            <div className="toggle-menu-container">
              ...
              >
                {toggleMenuOptions.map((option, index) => (
                  <div
                    key={index}
                    onClick={() => toggleMenuHandleOptionClick(option)}
                  >
                    {option}
                  </div>
                ))}
